...
import {
  ...
  AlertCircle, Brain, ClipboardList, Flame, Layers, Info, RefreshCw, HardHat,
} from "lucide-react";
...
const ISHIKAWA_CATEGORIES = [
  {
    ...
    causes: ["Equipe sem treinamento em PCIRO", "Não adesão às barreiras", "Falta de supervisão da obra"],
  },
  {
    id: "metodo",
    label: "Método",
    ...
    causes: ["Plano de controle ausente", "Risco não classificado (ICRA)", "Sem cronograma de inspeção"],
  },
  {
    id: "maquinas",
    label: "Máquinas/EPI",
    ...
    causes: ["Exaustor inoperante", "Sem pressão negativa", "EPI inadequado"],
  },
  {
    id: "material",
    label: "Material",
    ...
    causes: ["Barreiras improvisadas", "Filtros/HEPA ausentes", "Vedação insuficiente"],
  },
  {
    id: "medicao",
    label: "Medição",
    ...
    causes: ["Sem monitoramento de poeira", "Inspeções irregulares", "Sem cultura de fungos"],
  },
  {
    id: "meio_ambiente",
    label: "Meio Ambiente",
    ...
    causes: ["Dispersão de poeira", "Tráfego não controlado", "Entulho descoberto"],
  },
];
...
function IshikawaDiagram({
  ...
  return (
    <svg viewBox="0 0 900 400" className="w-full" style={{ minHeight: 340, maxHeight: 520 }}>
      ...
      <text x="850" y="186" textAnchor="middle" fontSize="9" fontWeight="700" fill="white">RISCO DE</text>
      <text x="850" y="199" textAnchor="middle" fontSize="9" fontWeight="700" fill="white">INFECÇÃO POR</text>
      <text x="850" y="212" textAnchor="middle" fontSize="8.5" fontWeight="700" fill="white">OBRAS</text>
      <text x="850" y="226" textAnchor="middle" fontSize="8" fill="white" opacity="0.85">EFEITO</text>
...
export default function DashboardConstruction() {
  ...
  const { stats, loading, items, allAudits } = useAuditDashboard("construction_renovation", { dia, mes, ano, setor });
  ...
  const derived = useMemo(() => {
    ...
    const DEMO_TOP_FAILURES = [
      { item: "Barreiras não protegem contra poeira",            count: 9, category: "Barreiras e Vedação" },
      { item: "Exaustor de ar ausente ou inoperante",            count: 7, category: "Controle de Poeira e Ar" },
      { item: "Portas sem vedação adequada",                     count: 6, category: "Barreiras e Vedação" },
      { item: "Contêineres de entulho descobertos",              count: 5, category: "Resíduos e Tráfego" },
      { item: "Área da obra sem limpeza regular",                count: 4, category: "Limpeza" },
      { item: "Sinalização de tráfego inadequada",               count: 3, category: "Resíduos e Tráfego" },
    ];
    const effectiveTopFailures = stats.topFailures.length > 0 ? stats.topFailures : DEMO_TOP_FAILURES;
    ...
    const barreirasCat = stats.categoryData.find((c) => /barreira|veda/i.test(c.name));
  ...
  const handleExportPdf = () => {
    ...
    exportPdf({
      ...
      data: {
        ...
        audits: stats.sectorData.map((s) => ({ type: "Obras/Reformas", sector: s.name, date: "", compliance: s.compliance, compliant: s.audits - s.nonCompliant, total: s.audits })),
      },
      filenamePrefix: "obras-reformas",
    });
  };
  ...
  const handleGerarPlano = () => {
    ...
    navigate("/quality/5w2h", {
      state: {
        prefill: {
          what: topFailure ? `Corrigir não conformidade em obra: ${topFailure.item}` : "Implementar medidas de controle de infecção em obras/reformas",
          why: `Conformidade atual em ${stats.avgCompliance}% (meta: 85%). ${stats.nonCompliantItems} itens não conformes identificados.${worstSector ? ` Local ${worstSector.name} com ${worstSector.compliance}% de conformidade.` : ""}`,
  ...
  const kpis = [
    ...
    { label: "Inspeções no Período", value: String(stats.totalAudits), sub: "registros analisados", icon: ClipboardCheck, color: "text-blue-600", bg: "bg-blue-50", border: "border-blue-200", status: null, statusColor: "" },
    ...
    { label: "Locais Adequados", value: String(derived.goodSectors), sub: `de ${stats.sectorData.length} locais`, icon: CheckCircle2, color: "text-emerald-600", bg: "bg-emerald-50", border: "border-emerald-200", status: null, statusColor: "" },
    { label: "Locais em Atenção", value: String(derived.warningSectors), sub: "75–84% conformidade", icon: AlertCircle, color: "text-amber-600", bg: "bg-amber-50", border: "border-amber-200", status: null, statusColor: "" },
    { label: "Locais Críticos", value: String(derived.criticalSectors), sub: "< 75% conformidade", icon: Flame, color: "text-red-600", bg: "bg-red-50", border: "border-red-200", status: derived.criticalSectors === 0 ? "OK" : "Alerta", statusColor: derived.criticalSectors === 0 ? "bg-emerald-100 text-emerald-800" : "bg-red-100 text-red-800" },
    ...
    <div className="space-y-5 md:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="flex items-center gap-2">
          <HardHat className="h-7 w-7 text-primary shrink-0" />
          <div>
            <h1 className="text-xl md:text-2xl font-bold">Dashboard — Obras/Reformas</h1>
            <p className="text-sm text-muted-foreground mt-0.5">
              Controle de infecção em obras e reformas · KPIs, OKRs e análise de causa raiz · Meta: ≥85%
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <DashboardAIInsights
            generateInsights={() => {
              const ins: string[] = [];
              ins.push(`📊 Conformidade geral de ${stats.avgCompliance}% com ${stats.totalAudits} inspeções no período.`);
              ins.push(`🏗️ ${derived.goodSectors} locais adequados, ${derived.criticalSectors} críticos, ${derived.warningSectors} em atenção.`);
              if (stats.nonCompliantItems > 0) ins.push(`⚠️ ${stats.nonCompliantItems} itens não conformes — risco de dispersão de poeira/fungos.`);
      ...
      <div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <OkrCard
            objective="O1 · Garantir conformidade ≥90% no controle de infecção em todas as obras"
            keyResults={[
              ...
              { label: "KR2: ≥4 inspeções/mês por obra", progress: derived.kr2Progress, status: derived.kr2Progress >= 75 ? "on_track" : derived.kr2Progress >= 50 ? "at_risk" : "off_track" },
              { label: "KR3: 0 não conformidades recorrentes", progress: derived.kr3Progress, status: derived.kr3Progress >= 80 ? "on_track" : derived.kr3Progress >= 60 ? "at_risk" : "off_track" },
            ]}
          />
          <OkrCard
            objective="O2 · Eliminar a dispersão de poeira e fungos a partir das áreas de obra"
            keyResults={[
              { label: "KR1: Barreiras/vedação ≥85%", progress: derived.kr4Progress, status: derived.kr4Progress >= 85 ? "on_track" : derived.kr4Progress >= 70 ? "at_risk" : "off_track" },
              { label: "KR2: Controle de poeira/ar ≥80%", progress: Math.round(derived.kr4Progress * 0.9), status: derived.kr4Progress * 0.9 >= 80 ? "on_track" : derived.kr4Progress * 0.9 >= 65 ? "at_risk" : "off_track" },
              { label: "KR3: Exaustores funcionais ≥95%", progress: derived.kr1Progress, status: derived.kr1Progress >= 90 ? "on_track" : derived.kr1Progress >= 75 ? "at_risk" : "off_track" },
            ]}
          />
          <OkrCard
            objective="O3 · Excelência em limpeza e gestão de resíduos durante as obras"
            keyResults={[
              { label: "KR1: Todos os locais ≥85%", progress: derived.kr5Progress, status: derived.kr5Progress >= 80 ? "on_track" : derived.kr5Progress >= 60 ? "at_risk" : "off_track" },
              { label: "KR2: Entulho sempre coberto", progress: Math.round(derived.kr3Progress * 0.85), status: derived.kr3Progress * 0.85 >= 75 ? "on_track" : "at_risk" },
              { label: "KR3: Limpeza com método aprovado ≥95%", progress: Math.round(derived.kr1Progress * 1.05) > 100 ? 100 : Math.round(derived.kr1Progress * 1.05), status: derived.kr1Progress >= 85 ? "on_track" : "at_risk" },
            ]}
          />
        </div>
      </div>
        ...
        <Card className="lg:col-span-2" ref={refs.evolucao}>
          <CardHeader className="pb-2 flex flex-row items-start justify-between gap-2 space-y-0">
            <div className="min-w-0">
              ...
              <CardDescription className="text-xs">Tendência vs meta · linha tracejada = referência</CardDescription>
            </div>
        ...
        <Card ref={refs.categoria}>
          <CardHeader className="pb-2 flex flex-row items-start justify-between gap-2 space-y-0">
            <div className="min-w-0">
              ...
              <CardDescription className="text-xs">% conformidade por grupo verificado</CardDescription>
            </div>
        ...
        <Card>
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-sm">Classificação de Risco por Local</CardTitle>
              ...
              {stats.sectorData.map((s) => (
                <div key={s.name} className="flex items-center justify-between p-3 rounded-lg border bg-card">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{s.audits} inspeç{s.audits !== 1 ? "ões" : "ão"}</p>
                  </div>
      ...
      {stats.totalAudits === 0 && (
        <Card className="border-dashed">
          <CardContent className="p-10 text-center">
            ...
            <p className="text-sm text-muted-foreground">Nenhuma verificação de obras/reformas registrada.</p>
